...
# Internal function that performs one MCMC iteration to sample from the posterior distribution of a decomposable graph. The R code of
# this function is a modified version of the Matlab code in Talhouk et al. (2012). For details, see Alexopoulos and Bottolo (2019)
...
generate_graph_zo <- function(g_current, delta, Phi, N, S_N, warmup, iters_to_keep)
{
  ...
  for (i in 1 : iter)
  {
    ...
    u <- runif(1)
    accept_prob <- 0
    xxx <- next_graph_candidate_zo(g_current, jtree, sepsize, cliques, reach_graph)
    g_proposal <- xxx[[1]]
    a <- xxx[[2]]
    b <- xxx[[3]]
    C <- xxx[[4]]
...
Sample_G <- function(E, Gcurr)
{
  ...
  ########## comment ##########
  # Hard coding! "iters_to_keep" is the internal number of iterations setup to generate a decomposable graph; the value of "warmup" 
  # is not relevant as defined originally on the Matlab code of Talhouk etal. (2012)
  
  g_next <- generate_graph_zo(g_current, delta, Phi, n, S_N, warmup, iters_to_keep)[[1]]
  output <- g_next
