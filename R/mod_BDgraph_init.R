...
# Internal function that generates an initial value of a non-decomposable graph. The R code of this function is a modified version of 
# the BDgraph R-package (Mohammadi and Wit, 2019). For details, see also Mohammadi and Wit (2015) and Alexopoulos and Bottolo (2019)


########## mod_BDgraph_init ##########

mod_BDgraph_init <- function(data, n = NULL, niter = 5000, burnin = niter /2, not.cont = NULL, g.prior = 0.5, 
                             df.prior = 2, 
                             cores = NULL, threshold = 1e-08)
{
  if (niter < burnin)
  {
